...
Да се најде бројот на патишта во графот, почнувајќи од фиксно теме V, кај кои сумата на темињата има вредност N. Темињата во патот може да се повторуваат.

Влез: Во првиот ред е даден бројот на рабови во графот M. Потоа во следните M редови се дадени рабовите во графот во формат теме1 теме2. 
...
public class Main {
    private static int sumOfVertices(AdjacencyListGraph<Integer> graph, int startVertex, int goalSum) {
        return sumOfVerticesR(graph, startVertex, goalSum, 0);
    }

    private static int sumOfVerticesR(AdjacencyListGraph<Integer> graph, int vertex, int goalSum, int sum) {
        sum += vertex;
        if (sum == goalSum){
            return 1;
        }
        ...
        for (int neighbor : graph.getNeighbors(vertex)){
            currentSum += sumOfVerticesR(graph, neighbor, goalSum, sum);
        }
        return currentSum;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sumOfVertices(graph, startVertex, goalSum));
