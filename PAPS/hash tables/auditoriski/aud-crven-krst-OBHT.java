...
class OBHT<K extends Comparable<K>, E> {
    ...
    public void insert(K key, E val) {
        ...
        if (existing_position != null)
            ...
            System.out.println("Failed to insert: Hash Table is full");
        }
    }
...
public class Main {
    public static void main(String[] args) {
        ...
        int N = sc.nextInt();
        OBHT<String,Integer> table = new OBHT<>(11);
        sc.nextLine();
        for (int i = 0; i < N; i++) {
            String []parts = sc.nextLine().split(" ");
            String krvnagrupa = parts[1];
            krvnagrupa = krvnagrupa.replaceAll("[1-2]","");
            if(table.search(krvnagrupa)==-1){
                table.insert(krvnagrupa, 1);
            }
            else{
                table.insert(krvnagrupa,table.getBucket(table.search(krvnagrupa)).value+1);
            }
        }
